...
\begin{align}
  \maximize_{c_{1}^{(1)} \in \reals^{p_{1}}, c_{1}^{(2)} \in
    \reals^{p_{2}}} &\frac{1}{n}c_{1}^{(1) T}X^{(1) T}X^{(2)}
  c_{1}^{(2)} \\
  \text{subject to } &\frac{1}{n}c_{1}^{(1) T}X^{(1) T}X^{(1)} c_{1}^{(1)} =
  \frac{1}{n}c_{1}^{(2) T}X^{(2) T}X^{(2)} c_{1}^{(2)} = 1.
\end{align}

\cite{witten2009penalized} argues for diagonalized CCA, in which the
variance constraints are replaced by unit norm constraints, as well as
the addition of sparsity-inducing $\ell^{1}$ constraints,
\begin{align}
  \maximize_{c_{1}^{(1)} \in \reals^{p_{1}}, c_{1}^{(2)} \in
    \reals^{p_{2}}} &\frac{1}{n}c_{1}^{(1) T}X^{(1) T}X^{(2)}
  c_{1}^{(2)} \\
...
\end{align}
which is exactly in the form \ref{eq:pmd_reform} where $X = X^{(1)
  T}X^{(2)}$, so indeed the PMD framework includes this form of sparse
CCA.
