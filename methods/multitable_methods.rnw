...
We emphasize that this report is reproducible.

<<overall-setup>>=
library("knitr")
library("ggplot2")
sim_methods_dir <- "~/Documents/programming/simFuns/simMethods/"
opts_knit$set(root.dir = sim_methods_dir)
opts_chunk$set(fig.width = 7.5, fig.height = 4, fig.align = "center",
               fig.show = "hold", size = "small", cache = FALSE)
theme_set(theme_bw())
...
CCA is a close relative of PCA that
explicitly compares sets of features across tables. Like PCA, it
provides low-dimensional representations of samples; however it
allows comparisons at the table level.
...
$v_{k} = \Sigma_{YY}^{-\frac{1}{2}}\tilde{v}_{k} \in
...
the two tables are closely related, then the $z_{k}^{(1)}$ and
$z_{k}^{(2)}$ will be very correlated. The singular values $d_{k}$
...
tables that can be captured by each additional pair of directions.

As in PCA, there are many ways to view this procedure; here we discuss
geometric, statistical, and probabilistic interpretations. Unlike the
geometric interpretation for PCA, the geometric interpretation for CCA
identifies each feature with a point, not each sample. Specifically,
the columns of $X$ and $Y$ are thought of as points in $\reals^{n}$. Consider the
two subspaces defined as the spans of the columns of $X$ and $Y$,
respectively. These subspaces correspond to the linear combinations of
features within each table. Place two ellipses on the
respective subspaces, centered at the origin and with size and shape
depending on the within table covariances $\hat{\Sigma}_{XX}$ and
$\hat{\Sigma}_{YY}$. The first canonical correlation directions are
the pair of points, one lying on each ellipse, such that the angle
from the origin to those two points is smallest. In this sense, it
finds a pair of linear combinations of features within the two tables
so that the two tables appear ``close''
...
product induced by the covariances in each table.
...
$\P^{\mathcal{Y}}$. The rows of the two tables are imagined to be i.i.d.
...
This perspective makes it easy to derive the algorithm given at the
start of this section. The empirical version of the optimization
...
Consider the transformed version of these variables, $\tilde{u} =
...
Consider arbitrary potential maximizers $\xi$ and $\nu$
of length one. We can find $w_{u}$ and $w_{v}$ such that $\xi =
\tilde{U}w_{u}, \nu = \tilde{V}w_{v}$, since $\tilde{U}$ and
$\tilde{V}$ are both orthonormal bases. Since they are length
...
To recover the solutions to the original problem, we reverse the
original variable transformation, yielding optimal $u^{\ast} =
\Sigma_{XX}^{-\frac{1}{2}}\tilde{u}^{\ast}$ and $v^{\ast} =
\Sigma_{YY}^{-\frac{1}{2}}\tilde{v}^{\ast}$.
...
and $y_{i}$ were drawn i.i.d. from the model,
\begin{align}
...
variable $\xi_{i}$, drawn from a spherical normal prior. A few of
the coordinates of these latent variables contribute to shared
structure, through $W_{X}$ and $W_{Y}$. The remaining coordinates
model table-specific structure, through $B_{X}$ and $B_{Y}$. The
authors demonstrated that the posterior expectations of the latent
$\xi_{i}^{s}$ (assumed $\in \reals^{K}$) given the observed tables
must lie on the subspace defined by the CCA directions. More
precisely,
\begin{align}
...
of how groups of species become more or less abundant depending on
environmental conditions in the sites at which they were observed
\cite{doledec1994co}. It can be viewed as a slight
modification\footnote{I might even call it a simplification.} of
CCA. Again, we seek sets of orthonormal
directions $\left(u_{k}\right)_{k = 1}^{K}$ and $\left(v_{k}\right)_{k
  = 1}^{K}$ such that the associated projections $Xu_{k}$ and
$Yv_{k}$ explain most of the covariation between the
tables. Unlike CCA, CoIA
maximizes the covariance -- not hte correlation -- between
scores. Formally, it chooses the first directions $u_{1}$ and $v_{1}$
as the solutions to
\begin{align}
...
derived directions, (i.e., $u_{k^{\prime}} \perp
\text{span}\left(\left(u_{k}\right)_{k = 1}^{k^{\prime} - 1}\right)$
and $v_{k^{\prime}} \perp \text{span}\left(\left(v_{k}\right)_{k =
    1}^{k^{\prime} - 1}\right)$).

Comparing with the CCA formulation \ref{eq:cancor_optim_emp}, we see that
the only difference is that the norm constraint is imposed on $u$ and
$v$ directly, rather than their transformations
$\Sigma_{XX}^{\frac{1}{2}}u$ and $\Sigma_{YY}^{\frac{1}{2}}v$. It is
in this sense that the CCA objective maximizes the correlation between
...
that reweights tables in a way that prevents any one table from
dominating the resulting ordination\footnote{Though we have not
  succeeded in demonstrating this phenomenon empirically, see Section
  \ref{sec:mfa_motivation_sim}.}. Specifically, MFA is a concatenated
PCA on the matrix
\begin{align}
...
variables before performing PCA.

The resulting MFA directions and scores can be interpreted in the same
way as those from PCA, in that, the MFA directions still specify the
...
$l^{th}$ table and the $k^{th}$ column of scores. According to this
definition, if the samples, as represented by the $l^{th}$ table, have
high correlation with the $k^{th}$ dimension of scores, then the
canonical analysis display positions the $l^{th}$ table far in the
$k^{th}$ direction.

\subsection{PCA-IV}

PCA-IV adapts the dimension reduction ideas of PCA to
multivariate regression setting \cite{rao1964use}. It can also be
viewed as a version of PCA that chooses a dimension reduction of $X$ in
light of its ability to predict $Y$, a kind of supervised
version of CCA. In this sense, it anticipates methods like Partial
Least Squares (PLS), Canonical Correspondence Analysis (CCpnA), the
Curd \& Whey procedure, and various approaches to multitask learning,
which are described in Sections \ref{sec:PLS},
\ref{sec:canonical-correspondence}, \ref{sec:cw}, and
\ref{sec:multitask}, respectively.

Formally, suppose we are predicting $y_{i}
\in \reals^{p_{1}}$ from $x_{i} \in \reals^{p_{2}}$. Since
$p_{2}$ may be large, it might be useful to work with a
lower-dimensional representation $z_{i} = V^{T}x_{i} \in
\reals^{K}$, that is potentially more interpretable and is still
predictive of $y_{i}$. As in PCA, we want the columns of $V$ to be
orthonormal.
...
minimize the residual covariance of $y_{i}$ given $z_{i}$. That
is, suppose that $y_{i}$ and $x_{i}$ are jointly normal with mean
0 (we can always center the data) and covariance
\begin{align}
...
If $z_{i} = V^{T}x_{i}$, then the joint covariance of
$y_{i}$ and $z_{i}$ is
\begin{align}
...
\begin{align}
  ...
  \tr\left(  \Sigma_{YY} -
  \Sigma_{YX}V\left(V^{T}\Sigma_{XX}V\right)^{-1}V^{T}\Sigma_{XY}\right). \label{eq:pca_iv_obj},
\end{align}
or, equivalently,
\begin{align}
  \maximize_{V\in \reals^{p_{2} \times K} \text{ orthonormal}}
  \tr\left(\Sigma_{YX}V\left(V^{T}\Sigma_{XX}V\right)^{-1}V^{T}\Sigma_{XY}\right). \label{eq:pca_iv_obj_2},
\end{align}

The optimal $V$ are the top $k$ generalized eigenvectors of
$\hat{\Sigma}_{XY}\hat{\Sigma}_{YX}$ with respect to
$\hat{\Sigma}_{XX}$, that is, the orthonormal set of
$\left(v_{k}\right)$ satisfying
\begin{align}
...
\Sigma_{XX}^{\frac{1}{2}}\tilde{v}$. But to maximize
...
$\Sigma_{XY}\Sigma_{YX}$ with respect to $\Sigma_{XX}$. Indeed, in
this case,
\begin{align}
...
Hence, in the case $K = 1$, the criterion is maximized by the top
generalized eigenvector. For larger $K$, recall that the
problem of maximizing $\frac{v^{T}Av}{\|v\|^{2}}$ over $v$ subject to
being orthogonal to the first $K - 1$ eigenvectors of $A$ is solved by
the $K^{th}$ eigenvector of $A$, and applying this fact in step
\ref{eq:gev_opt_2} of the argument above gives the result for general
$K$.

For a geometric interpretation of PCA-IV, view each
column $y_{\cdot j}$ in $Y$ and $x_{\cdot j}$ in
$X$ as a point in $\reals^{n}$. Assuming $X$ and $Y$ are full rank,
the collections $\left(y_{ \cdot j}\right)$ adn $\left(x_{\cdot
  j}\right)$ span $p_{1}$ and $p_{2}$-dimensional subspaces.
A set of independent regressions of the $X$ onto the $y_{\cdot j}$
simply projects the $y_{\cdot j}$ onto the span of $\left(x_{\cdot
  j}\right)$, and the residuals are the distance to this
span. The PCA-IV procedure is an attempt to find a further
$K$-dimensional subspace within the span of the $\left(x_{\cdot
  j}\right)$ such that the residuals of the regressions from
$x_{\cdot j}^{(1)}$ onto this further subspace is not much worse.

Indeed, write the usual estimates for the covariance matrices of
interest,
\begin{align}
  \Sigma_{YY} = \frac{1}{n}Y^{T}Y \\
  \Sigma_{YX} = \frac{1}{n}Y^{T}X \\
  \Sigma_{XX} = \frac{1}{n}X^{T}X
\end{align}
...
$Y$ onto the span of the PCA-IV subspace, justifying the
...
multitable data when each table has the same dimension
\cite{thioulouse2011simultaneous}. Specifically,
...
called a data cube because it can also be written as a three-dimensional
array $X \in \reals^{n \times p   \times L}$. We denote the $j^{th}$
feature measured on the $i^{th}$ sample
...
we measure the same environmental variables at the same locations across a
...
the compromise table. A naive approach would simply average each
...
-\frac{1}{2\lambda} \left<\overline{X}, X_{\cdot\cdot l}\right>$ for
all $l$. The
...
\frac{\left<\overline{X}, X_{\cdot\cdot l}\right>}{\sqrt{\sum_{l^{\prime}
      =1}^{L}\left<\overline{X},
      X_{\cdot\cdot l^{\prime}}\right>^{2}}}$. Therefore, we can write
...
\end{align}
In contrast to the PTA objective, this differentially penalizes
the $\alpha_{l}$'s depending on the norms and inner products of the
$X_{\cdot\cdot l}$.
...
X \in \reals^{Ln \times p} = \begin{pmatrix} x_{\cdot 1 1} &
  \dots & x_{\cdot pl} \\ \vdots & & \vdots \\ x_{\cdot 1 l}
  & \dots & x_{\cdot p l} \end{pmatrix}.
\end{align}

Perhaps the reason for avoiding this approach is that it requires
inverting an $Ln \times p$ matrix.
...
over time. We write these are $Y \in \reals^{n \times p_{1} \times L}$
and $X \in \reals^{n \times p_{2} \times L}$. Costatis and Statico are
two approaches for analyzing such data
\cite{thioulouse2011simultaneous}. They are easiest to understand as
divide-and-conquer approaches, where the general problem of analyzing
a pair of data cubes is divided into steps adapted to analyzing single
cubes and pairs of (two-dimensional) tables. In Statico, the paired
aspect of the data is dealt with first and followed by a data cube
analysis, while in Costatis, the cube element of the data are studied
first, and followed-up with a paired analysis. This is described more
precisely below.

In Statico, an empirical cross-covariance matrix is constructed at
each time point, $Z^{l} = \frac{1}{n_{l}}Y^{T}_{\cdot\cdot
  l}X_{\cdot \cdot l}$. This is, for example, the objective
...
of the data cubes $Y$ and $X$, using PTA, call these $Y_{c}$ and $X_{c}$. These are now
...
which ties the different responses
together\footnote{izenman1975reduced, mukherjee2011reduced}. Compared
to treating each individual response as its own univariate regression
problem, this pooling across responses can lead to meanintful
performanceimprovements. Further, we will see that the reduced-rank
point of view can aid interpretation as well.

Our setup is that we have collected $p_{1}$ responses and $p_{2}$
features across $n$ samples, $y_{i} \in \reals^{p_{1}}$ and $x_{i} \in
...
correlated. The intuition is that while there may be $p_{1}$
responses, their effective dimension may be relatively low.
...
The optimal value is given by $\hat{B}^{\text{ols}}V_{K}V_{K}^{-}$, where
the columns of $V_{K}$ are the top $K$ response CCA directions and
$V_{K}^{-1}$ denotes their pseudoinverse. To see this, consider
...
$\hat{Y}^{\ast \text{ols}} = \dot{U}\dot{D}\dot{V}^{T}$. We know that the
...
Hence, the reduced-rank coefficients are just the projection of
the original OLS coefficients onto the subspace spanned by the top $K$ response
canonical directions.

Similarly, we can view directly
$\hat{Y}^{\text{rr}} = X\hat{B}^{\text{rr}} = P_{X}YV_{k}V_{k}^{-}$, which
...
been pooled, then the projection onto the span of the $X$'s is exactly the
independent linear regressions approach described above.
...
this trend: Partial Least Squares (PLS) is well-suited for
high-dimensional spectra (which explains its popularity in
chemometrics), while Canonical Correspondence Analysis (CCpnA) was
...
the relationship between two tables, $Y$ and $X$
...
matrix between the two tables, $\hat{\Sigma}_{YX} =
\frac{1}{n}Y^{T}X$. Then, for each of the $p_{2}$ columns of
$X$, compute the univariate (i.e., partial) regression coefficient
$\hat{\varphi}_{j}$ from the model $u_{1i} = \alpha_{0j} +
\varphi_{j}x_{ij}$, for $i = 1, \dots, j_{1}$. The first PLS
direction is defined as $z_{1} = X\hat{\varphi}_{1}$. To
generate subsequent directions, orthogonalize both $Y$ and
$X$ with respect to the current directions, and repeat the process.
...
geometrically or probabilistically. Subsequently, several authors have
attempted to provide statistical interpretations for the PLS
directions. \cite{frank1993statistical} and \cite{stone1990continuum} studied
the case where $p_{1} = 1$, so $y$ is a single column
vector. They further suppose the rows of $y$ and $X$ are
drawn i.i.d. from distributions $\P^{y}$ and $\P^{X}$. They found
that the $k^{th}$ PLS direction $z_{k}$ is the $z$ that solves the
optimization
\begin{align}
\max_{z} \medspace & \Corrsubarg{\P^{X} \times \P^{y}}{x_{i}^{T}z_{k},
y_{i}}\Varsubarg{\P^{X}}{z^{T}x_i} \\
\text{such that }&z^{T}y^{T}y z_{j} = 0 \text{ for all }j
\leq k - 1 \text{ and }
\|z\| = 1.
\end{align}
If the covariance term is omitted, the optimization is identical to
the maximum variance problem that gives the principal component
directions based on $X$. This formulation makes precise the idea
...
\left(\xi_{i}^{s}, \xi_{i}^{X}\right)$ are drawn i.i.d. from a $K_{1} +
K_{2} = K$ dimensional spherical normal. The observed tables $Y$
and $X$ have rows drawn i.i.d. from
\begin{align}
...
CCpnA is a two table method designed to answer questions in ecology
involving joint analysis of count and continuous data. For example
$Y \in \reals^{n \times p_{1}}$ might be a table whose rows are ecological sites, and whose
columns are the counts of different species at those sites, while
$X \in \reals^{n \times p_{2}}$ might describe environmental characteristics at those sites
...
CCpnA produces low-dimensional representations for both the rows and
columns of $Y$ (the sites and species), as well as the latent
subspaces onto which these representations are
projected. Algorithmically, CCpnA first constructs the
following matrices, where $1_{r}$ denotes a column vector of $r$ ones,
\begin{enumerate}
  \item An overall frequency matrix,
    \begin{align}
      F =
    \frac{1}{n_{\cdot\cdot}^{(1)}} Y,
  \end{align}
...
proposal developed CCpnA as a solution to a fixed-point iteration
called reciprocal averaging; they
also derived it as the approximate MLE of a probability
model \cite{ter1986canonical}. Later, \cite{greenacre1987geometric,
  greenacre1984theory}, provided a geometric view, \cite{yee2004new}
designed a method for obtaining the exact MLE, and
...
describing implicit probability models behind different
...
As is, this is not quite the setup that leads to CCpnA\footnote{It in
  fact gives the solution to the Correspondence Analysis problem (the
  similarity is the reason for the name Canonical
  \textit{Correspondence Analysis}).} -- it doesn't even use information in the
environmental variables table $X$. To recover CCpnA, a projection
...
in a one-dimensional score for rows and columns. Let $\alpha$ be a
...
$f_{j}\left(x_{i}\right) = \Gsn_{p^{(2)}}\left(\mu_{j},
...
first $K$ response canonical correlation directions can be viewed as the
hard-thresholding analog. In this way, C\&W is to reduced-rank
regression what ridge regression is to principal component
regression.
...
\cite{breiman1997predicting} provide several options. One nice
...
define new fitted values from a linear combination of independent
...
\begin{align}
  \hat{B}^{\text{cw}} := \arg\min_{B} \sum_{i = 1}^{n} \|y_{i} - B
  \hat{y}_{-i}\|_{2}^{2}. \label{eq:cv_looc}
\end{align}

Here, $\hat{y}_{-i}$ is the OLS fitted value for $y_{i}$ when training
on all samples but the $i^{th}$. For ease of notation, we write
$\hat{y}$ instead of $\hat{y}^{\text{ols}}$, referring explicitly to
$\hat{y}^{\text{c\&w}}$ when needed.
...
elements of the projection matrix $P_{X}$ and $\hat{y}_{i}$ is
...
be deduced directly. This can dramatically speed up computation of the
sum in \ref{eq:cv_looc}. While this does not resolve the question of how to
manage the search over possible $B$, concrete proposals are discussed
in \cite{breiman1997predicting}.
...
Several different literatures within machine learning have developed
approaches to dimensionality reduction across multiple
...
clear what their overall utility will be in the data analysis we care
most about.
...
More precisely, let $\varphi^{Y}: \reals^{p_{1}} \to H^{Y}$ and
$\varphi^{X}: \reals^{p_{2}} \to H^{X}$ be mappings from the space
associated with table $l$ to a richer Hilbert space. For example,
the $\varphi$'s might map vectors into an expansion of all polynomial
products of coordinates, up to some fixed kernel; this is mapping is
called the polynomial kernel. As in CCA, let $\P^{X}$ and $\P^{Y}$
denote the sampling distributions associated with the two tables; let
$x_{i}$ and $y_{i}$ denote generic draws from these tables.

In the same way that the first CCA direction maximizes the covariance
between linear combinations $u^{T}x_{i}$ and  $v^{T}y_{i}$ (subject to
a variance constraint), KCCA maximizes the correlation between linear
functionals, $z_{i}\left(u\right) = \left<u, x_{i}\right>$ and
$z_{i}\left(v\right) = \left<v, y_{i}\right>$.
\begin{align}
  \argmax_{u \in H^{X}, v \in H^{Y}}
  &\Covsubarg{\P^{X} \times \P^{Y}}{z_{i}\left(u\right), z_{i}\left(v\right)} \\
\text{subject to } &\Varsubarg{P^{X}}{z_{i}\left(u\right)} &=
\Varsubarg{P^{Y}}{z_{i}\left(v\right)} = 1. \label{eq:optim_kcca}
\end{align}
...
\begin{align}
  ...
  \frac{\lambda^{X}}{2}Pen\left(u\right) +
  \frac{\lambda^{(Y)}}{2}Pen\left(v\right) \label{eq:lagrangian_kcca}
\end{align}

Now, note that the optimal $u$ and $v$ must lie in
the spans of $\left(\varphi^{X}\left(x_{i}\right)\right)_{i =1
}^{n}$ and $\left(\varphi^{Y}\left(y_{i}\right)\right)_{i =1 }^{n}$, respectively,
...
becomes clear that only the cross-products $\Phi^{X T}\Phi^{X}$,
and $\Phi^{Y T}\Phi^{Y}$ appear. Since these inner products can be
written as kernel matrices -- call them $K^{X}$ and $K^{Y}$ --
we have that the optimization can be fully kernelized.  It can then be
shown that the optimal $\alpha^{X}$ and $\alpha^{Y}$ are the
...
of average pairwise covariance, discussed at the end of Section
\ref{sec:cca}. Formally, we let $z_{i1}^{(l)} = \left<c^{l},
...
\sum_{l = 1}^{L}\frac{\rho^{(l)}}{2}\Varsubarg{P^{(l)}}{z_{i1}^{(l)}} -
 \sum_{l = 1}^{L} \frac{\lambda^{(1)}}{2}Pen\left(c^{l}\right) \label{eq:lagrangian_mkcca},
\end{align}
over different values for the $c^{l}$.
...
\end{pmatrix} &=
\rho \begin{pmatrix}
  \left(K^{(1)} + \lambda_{1}I_{p_{1}}\right)^{2} & \dots & 0 \\
  \vdots & \ddots & \vdots \\
  0 & \dots & \left(K^{(L)} + \lambda_{L}I_{p_{L}}\right)^{2}
\end{pmatrix}
\begin{pmatrix}
  \alpha_{1} \\
  \vdots \\
...
\end{pmatrix}
\end{align}
whose solution \cite{yamanishi2003extraction}calls ``multiple kernel
...
computed in the Hilbert spaces $H^{X}$ and $H^{Y}$, so it's
impossible to make a biplot). However, the KCCA scores can be
interpreted using supplementary features, as in any other
dimensionality reduction method.

For the record, we note that \cite{lanckriet2004statistical} developed
a complementary formulation of Kernel CCA.

\paragraph{Simulation}

To ground this discussion, we provide two simulations comparing CCA to
Kernel CCA. The first simulation generates data lying on concentric
circles around the origin, similar to examples in Chapter 14 of
\cite{friedman2001elements}, while the second generates data well
suited to the RBF kernel, and is based on an experiment in
\cite{fukumizu2007statistical}.

We first discuss the circular data example. The data generating
mechanism is as follows. There are two data sets $X \in \reals^{n
  \times 2}$ and $Y\in\reals^{n \times 2}$ on the same $n = 100$
samples. Each sample belongs to one of two clusters, denoted by $z_{i}
\in \{0, 1\}$. Samples in cluster 0 fall closer to the origin for both
tables, while the angles are random and unrelated to cluster
status. The resulting data is displayed in Figure \ref{fig:kcca-data-concentric}.

Specifically, we draw from a hierarchical model,
\begin{align}
  \omega^{X}_{i} &\sim \Bet\left(1, 4\right) \\
  r_{i}^{X} &\sim \Unif\left(\left[\frac{1}{2},
    \frac{3}{2}\right]\right)\indic{z_{i} = 0} +
  \Unif\left(\left[\frac{3}{2}, \frac{5}{2}\right]\right)\indic{z_{i}
    = 1} \\
  \theta_{i}^{X} \sim \Unif\left(\left[2 \pi \omega_{i} - \frac{1}{2},
    2 \pi \omega_{i} + \frac{1}{2}\right]\right)
\end{align}
for table $X$, and
\begin{align*}
  \omega^{y}_{i} &\sim \Unif\left(0, 1\right) \\
  r^{Y}_{i} & \sim \sqrt{\Unif\left(\left[\frac{1}{2},
      \frac{3}{2}\right]\right)}\indic{z_{i} = 0} +
  \sqrt{\Unif\left(\left[\frac{3}{2},
      \frac{5}{2}\right]\right)}\indic{z_{i} = 1} \\
  \theta^{Y}_{i} &\sim \Unif\left(\left[0, 2\pi\omega_{i}\right]\right),
\end{align*}
for table $Y$. The resulting points are plotted in polar coordinates
as $\left(r_{i}^{\cdot}, \theta_{i}^{\cdot}\right)$.

We highlight one subtlety with preparing kernel CCA experiments that
is not an issue with kernel PCA experiments --
nonlinearity within each tables does not imply nonlinearity in the
relationship across tables. Although the concentric circles make the
data within tables very nonlinear, it is the nonlinear relationship
across tables induced by the associated generation mechanism that is
of primary interest.

The code below generates the simulation data and produces Figure
...
Next, we perform KCCA as described above, using the implementation in
\texttt{kernlab}\footnote{But for an implementation
  that allows for more than two tables see our experiments at
  \url{https://github.com/krisrs1128/kcca\_expers}}
\cite{zeileis2004kernlab}.
...
$k\left(x, x^{\prime}\right) = \exp{-\sigma\|x- x^{\prime}\|^{2}}$,
  ...
  \footnote{But called $\lambda$ in \texttt{kernlab}.}, we set it to
  ...
  large; we are not aware of any nonsubjective ways for parameter
  tuning in unsupervised kernel methods.
...
As a reference point, we also apply standard CCA to this data set. Note that it requires no tuning.
...
We have also implemented a second simulation using KCCA, based on the
experiment discussed in \cite{fukumizu2007statistical}. As it is
similar to the example discussed here, we relegate it to Section
\ref{sec:kcca_second} in the Appendix.
...
\begin{table}
  ...
  \begin{tabular}{|m{8cm}|m{8cm}|}
    ...
    a normal with mean zero and variances $\texttt{sigma\_{b}}^{2}$. It may be more
    ...
    mean zero and variances $\texttt{sigma\_{s}}^{2}$. \\
    \hline
    \texttt{sigma\_y} & For each task, the regression data is
    $y^{(r)}_{i} \sim x^{(r) T}_{i}\beta^{(r)} + \eps_{i}^{(r)}$, where
...
<<plot-B>>=
@
\caption{The underlying coefficient matrix. The $\left(j,
  r\right)^{th}$ cell displays the value of $\beta_{j}^{(r)}$.}
\label{fig:vb-plot-B}
\end{figure}
...
$n$ or larger $\sigma\_{b}^{2}$, the estimation procedure often
...
variance\footnote{We use the $V_{jj}^{(r)}$ to construct confidence
  intervals for estimated parameters, though it is well-documented
  that variational bayes systematically underestimates the posterior
  variance of different parameters \cite{consonni2007mean}.} of the
$\beta_{j}^{(r)}$. As a point of reference,
...
For a final view of the estimated $\hat{beta}_{j}^{(r)}$, we generate
the analog of Figure \ref{fig:vb-plot-B}, which showed the true
coefficient matrix across all features and tasks. The result is in
Figure \ref{fig:vb-plot-betas-image}. This figure again reinforces the
fact that the linear model estimates can be occasionally be quite
extreme, while the variational estimates are quite
regularized. Further, we notice that the task clustering structure was
recovered by both approaches, even the independent regressions
approach, which did not specify this structure a priori.

\begin{figure}
<<plot-betas-image>>=
@
\caption{The estimated coefficient matrices for the Bayesian multitask
  and independent linear regressions approaches. Note that the tasks
  have been reordered to the coefficient clustering structure more
  visible.}
\label{fig:vb-plot-betas-image}
\end{figure}
...
\subsection{A Second KCCA Simulation}
\label{sec:kcca_second}
