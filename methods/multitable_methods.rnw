...
drawn i.i.d. from distributions $\P^{(1)}$ and $\P^{(2)}$. They found
...
\left(\xi_{i}^{shared}, \xi_{i}^{(2)}\right)$ are drawn i.i.d. from a $K_{1} +
K_{2} = K$ dimensional spherical normal. The observed tables $X^{(1)}$
and $X^{(2)}$ have rows drawn i.i.d. from
\begin{align}
...
\subsubsection{Canonical Correspondence Analysis (CCpnA)}
...
projected. Algorithmically, CCpnA proceeds first constructs the
...
proposal described CCpnA as a solution to a fixed-point iteration; they
...
environmental variables table $X^{(2)}$. To recover CCpnA, a projection
...
The iteration now converges to the previously described CCpnA solution.
