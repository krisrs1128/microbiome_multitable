...
Here, we provide two simulations comparing CCA to Kernel CCA,
\begin{itemize}
\item Circular data: There are two data sets $X \in \reals^{n \times 2}$ and
  $Y\in\reals^{n \times 2}$ on the same $n$ samples. Each sample belongs to
  one of two clusters, indicated by $z_{i} \in \{0, 1\}$. Samples in cluster
  0 fall closer to the origin for both tables, while the angles are random
  and unrelated to cluster status. Specifically, using polar coordinates,
  we draw
  \begin{align}
...
To ground the discussion, consider the following simulation.
