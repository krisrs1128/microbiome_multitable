...
We generate data according to the clustered regression scheme. We suppose there
are $p_{1} = 100$ tasks, and each is randomly (and uniformly) assigned to one of
three task  clusters. These are the $w^{(r)}$ from before. For each task,
we generate data according to a linear model with its own coefficient, 
but so that the coefficients are close whenever the tasks belong to the same
task cluster. Each individual regression problem has 15 samples with 10-dimensional
measurements. Recalling our earlier notation, we set
\begin{align}
\beta^{(r)} = Sw^{(r)} + \Psi,
\end{align}
where $\Psi \sim \Gsn\left(0, I_{10}\right)$ and
$s_{i\cdot} \stackrel{iid}{\sim} \Gsn\left(0, 2^{2}I_{3}\right)$\footnote{Though
  the model treats $S$ as fixed, we still need some way of simulating it.}. The
regression data is then $y_{i}^{(r)} = x_{i}^{(r)} T} \beta^{(r)} + \eps_{i}$ for
  $i = 1, \dots, 15$ and $\eps_{i} \sim \Gsn\left(0, 2^{2}\right)$.

This information is summarized in Figures \ref{fig:...} and \ref{fig:...}



To interpret this in a microbiome context, for example....

illustrate the setting 
in which this method is useful

- Describe setup
    + explain how this might be related to the microbiome
- Visualize B matrix
    + explain why clustered
    + explain why not exactly clustered
- Visualize example regressions
    + what are rows and cols
    + why this should be hard for naive regression problem
- Explain results
    + Show fitted vs. true B
    + they both implicitly recover cluster structure, but bayesian approach
      regularizes, has somewhat less variance
    + bayesian approach explicitly returns fitted probabilities, without using 
      any extra modeling
- Show the fitted regression lines  
